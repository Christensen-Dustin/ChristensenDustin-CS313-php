...
<h1>User Details</h1>
<?php
foreach ($db->query("select parent_pk, parent_fname, parent_lname, parent_bday, parent_display from parent where parent_pk='$userName'") as $rowParent)
{
    echo 'User Name: ' . $rowParent['parent_fname'] . ' ' . $rowParent['parent_lname'] . '</br>';
    echo 'Birthday: ' . $rowParent['parent_bday'] . '</br>'; 
}

foreach ($db->query("select child_pk, child_fname, child_lname, child_bday, child_display from child where child_pk='$userName'") as $rowChild)
{
